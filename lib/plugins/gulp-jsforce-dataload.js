...
module.exports = (options,mode) =>{
  return through.obj((file, enc, callback) => {
    ...
    } catch(err) {return callback(new gutil.PluginError(PLUGIN_NAME + ' ' + mode,'File name not parsable'));}
    ...
    conn.login(options.username, options.password, (error, userInfo) => {
      ...
      conn.bulk.load(objecttype, mode, file, (err, rets) => {
        if (err) { return callback(new gutil.PluginError(PLUGIN_NAME + ' ' + mode,error)); }
        for (let res of rets) {
          if (res.success) {
